...
### T8 — FE: Mailcow-Admin-Panel (Domains/Mailboxen/ACL/Delegates)  [ ] IN ARBEIT (Teil 1 ✓ 49ec200b8 · 2a ✓ 3aaeb52ac · 2b-i ✓ Create 00ec28fc6 · 2b-ii-A ✓ Edit/Delete 8e63ea095 · 2b-ii-B offen: ACL-Editor + Doku)
...
> **Teil 2b-ii-B OFFEN: ACL-Editor + Doku (schließt T8 ab).** ACL-Editor-Dialog (per-Row-Aktion „ACL" ergänzen, oder aus dem Edit-Dialog): baut `MailboxAclDto {items:[username], attr:{user_acl:string[]}}`, Checkbox-Gruppe über `MAILCOW_ACL_OPTIONS` (libs-Konstante aus T7, 14 Werte), ruft `updateMailboxAcl` (boolean-Return). i18n `mailcowAdmin.*` ACL-Labels DE/EN/FR + **kurze Doku** `docs/` (DE+EN+FR: was das Mailcow-Admin-Panel kann, wo es liegt, Env `MAILCOW_API_URL`/`MAILCOW_API_TOKEN`). **Delegates weglassen** (Phase-3-deferred). Danach **T8 komplett** → Phase-2-Abschluss-Check (iter.sh all / path-gated Voll-Stack — box-gated) + PR-Gate parken. Verify = eslint + isolierter FE-tsc + vitest (ACL-Auswahl-Logik pur); Interaktion+Visual box-gated.
