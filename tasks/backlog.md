...
### T15 — Voll-Stack-Verify gegen echten LMN (Accept→Gruppe, Reject→Entfernung)  [?] WEITGEHEND VERIFIZIERT, Accept-Schritt LMN-seitig blockiert (2026-07-27). **Belegt gegen echten LMN** (crabbox lmnui-1d1e, Schueler `verifyan` + Lehrer `rabe` als elternfaehiges Konto — `getIsParent` behandelt TEACHER/STAFF explizit als elternfaehig): Schueler erzeugt Kopplungscode (`GET code`, 8 Zeichen) **OK**; Elternteil loest ein (`POST /`) **201** → Datensatz `{parent:rabe, student:verifyan, school:default-school, status:pending}` inkl. Audit-Log `pairing_requested`; Admin sieht ihn ueber `GET all` als **pending**; `GET relationships` liefert die Beziehung; Guard unauth → **401**. **Accept scheitert mit 502 `AddParentToStudentFailed`** — NICHT durch falschen Nachbau: gegen die OpenAPI von linuxmuster-api7 7.3.35 verifiziert, dass unser Aufruf vertragstreu ist (`POST /v1/users/{user}/parents` existiert, Body-Schema `UserList` = `{users:[...]}`, exakt was `LmnApiService.addParentToStudent` sendet; `DELETE` analog fuer Reject). **Ursache liegt am LMN-Datenstand:** dieser Server hat **keine Eltern-Konten** (`role-parent`- und `<klasse>-parents`-Gruppen existieren, sind aber leer; sophomorix bietet hier keine parent-Rolle), und api7 lehnt einen Lehrer als Elternteil ab. **Kein LMN-Zustand wurde veraendert** (der Accept schlug fehl, bevor etwas geschrieben wurde). **Zum Abschluss noetig:** ein echtes Eltern-Konto auf dem LMN; damit sind Accept→Gruppe `<student>-parents` und Reject→Entfernung in einem Lauf nachholbar. Beantwortet zugleich Offene Frage 2 teilweise: Eltern brauchen ein eigenes Konto mit `role-parent`, ein Lehrer-Konto genuegt der LMN-Seite nicht.
