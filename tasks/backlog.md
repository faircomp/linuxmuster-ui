...
### T8 — FE: Mailcow-Admin-Panel (Domains/Mailboxen/ACL/Delegates)  [ ] IN ARBEIT (Teil 1 ✓ Store 49ec200b8 · 2a ✓ read-only Panel 3aaeb52ac · 2b-i ✓ Create-Dialog 00ec28fc6 · 2b-ii offen: Edit/Delete/ACL)
...
> **Teil 2b-ii OFFEN: Edit/Delete/ACL.** Aktions-Spalte in der Mailbox-Tabelle (TableActionCell wie getParentAssignmentColumns: Edit/Delete pro Zeile) + **EditMailboxDialog** (baut UpdateMailboxDto {items:[username], attr:UpdateMailboxAttrDto}; name/quota/active/tls_* editierbar, Passwort optional) + **Delete** via `DeleteConfirmationDialog`-Reuse (wie DeleteMailProviderConfigDialog, ruft deleteMailcowMailboxes([username])) + **ACL-Editor** (MailboxAclDto {items,attr:{user_acl}}, Multi-Select/Checkbox-Gruppe aus `MAILCOW_ACL_OPTIONS`-libs-Konstante). Rufen die (nun boolean-returnenden) Store-Mutationen. i18n `mailcowAdmin.dialog.*` Edit/ACL-Labels DE/EN/FR + kurze Doku (docs/, DE+EN+FR). **Delegates weglassen** (Phase-3-deferred). Verify = eslint + isolierter FE-tsc + vitest (Edit-Validierung pur); Interaktion+Visual box-gated.
