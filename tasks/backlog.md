...
### T8 — FE: Mailcow-Admin-Panel (Domains/Mailboxen/ACL/Delegates)  [ ] IN ARBEIT (Teil 1 ✓ Store 49ec200b8 · Teil 2a ✓ read-only Panel 3aaeb52ac · Teil 2b offen: CRUD+ACL)
...
> **Teil 2b OFFEN: CRUD-Dialoge + ACL-Editor.** Create/Edit/Delete-Mailbox-Dialoge (bauen die T6-DTOs CreateMailboxDto/UpdateMailboxDto/DeleteMailboxesDto; Domain-Dropdown aus `mailcowDomains`; Passwort+Passwort2, quota, active/force_pw_update/tls_* — client-seitige Vorvalidierung spiegelt die BE-Härtung) + Aktions-Spalte/Buttons in der Tabelle (Edit/Delete) + ACL-Editor (MailboxAclDto, Multi-Select aus `MAILCOW_ACL_OPTIONS` — libs-Konstante aus T7 existiert bereits) → rufen die Teil-1-Store-Mutationen. i18n `mailcowAdmin.*` Dialog-Labels/Felder DE/EN/FR; kurze Doku (docs/, DE+EN+FR). **Delegates weglassen** (Phase-3-deferred, kein BE-Route). Verify = eslint + isolierter FE-tsc + vitest (Dialog-Validierungslogik pur extrahieren); Interaktion+Visual box-gated.
