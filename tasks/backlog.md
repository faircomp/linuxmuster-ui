...
- **2.1.0-DRIFT-ANALYSE + SICHERHEITS-ARBEIT (2026-07-27)** — systematisch, 7 Achsen + 3 adversariale Sicherheits-Lupen, 141 Port-Items. Referenz `.reference/2.1.0/` neben `.reference/2.0.200/` erfasst (`fetch-reference.sh` ist dafuer jetzt versionierbar; Default bleibt 2.0.200, Versions-Pin bricht sauber ab).
  - **Umfang 2.1.0:** `main.js` 73 240 → **94 907 Zeilen (+30 %)**; +2 Module (`AiModule`, `ContactsModule`), **+62 Routen**, +19 Injectables, +11 Schemas, **11 Migrationen**, kompletter Sieve-Stack fuer Mail. Entfernt: `CalendarShareEntry`, `Survey.backendLimiters`, `ciLightBlue`, 4 Routen-Shapes. Toolchain: React 18→19, ESLint 8→9 (gatet jeden 2.1-FE-Nachbau).
  - **KERNBEFUND: drei aktiv ausnutzbare Luecken, zwei davon REGRESSIONEN UNTER dem eigenen 2.0.200-Ziel** — der Nachbau hat Schutzmassnahmen verloren, die 2.0.200 hatte. Das rechtfertigt den Guardrail „Auth-Guards nie verlieren" rueckwirkend.

  **GEFIXT in dieser Session (alle remote verifiziert, gepusht):**
  - `fix(mails)` **e72d6e7ba** — (a) **IDOR**: `deleteSyncJobs` trug nur den Kommentar „NIEDUUI-374: Check if user has permission" und pruefte nie → jeder User konnte fremde Sync-Jobs loeschen; 2.0-Eigentuemerpruefung portiert (403 `SyncJobAccessDenied`) + 5 Regressionen. (b) **Provider-Config-Leak**: `GET /mails/provider-config` gab jedem User Host/Port/Encryption; 2.0 hat dort `AdminGuard`. Naiver Guard haette die Nutzerseite zerschossen (sie baute den Sync-Job clientseitig) → 2.0-Entwurf portiert: oeffentliche Route (nur id/name/label) + AdminGuard auf der Vollversion + **serverseitige Provider-Aufloesung** aus `mailProviderId`. Contract-Spec hatte die Luecke als korrekt festgeschrieben.
  - `fix(security)` **61cb5b889** — (a) **SSE-Channel-Hijack, unauthentifiziert**: authentifizierter Kanal per Benutzername geschluesselt, `@Public()`-Login-Route schob ungepruefte `sessionId` in dieselbe flache Map → `GET /sse/auth?sessionId=<username>` haengt am Stream des Opfers; ueber genau diesen Kanal laeuft der QR-Login mit `{username,password}`, und ein Verbindungsabbruch beendet das Subject des Opfers (DoS). 2.1-Namespacing portiert (`login-session:`/`public-conference:` an **beiden** Enden, 403 bei Doppel-Abonnent, `isNonUserChannel` haelt Fremdschluessel aus dem Praesenz-Cache) + neuer Key `auth.errors.Forbidden` DE/EN/FR. (b) **Whiteboard-Path-Traversal (Regression)**: 2.0.200 hat `ValidatePathPipe` auf **beiden** Asset-Routen (main.js 55938/55950), der Fork nicht — obwohl die Pipe samt Tests im Repo liegt. Lesen **und Loeschen** beliebiger Dateien im API-Container. Pipe angeschlossen + `WHITEBOARD_FILES_PATH`.
  - `fix(appconfig)` **ab884c4dc** — `GET /appconfig/public(/:name)` gab als `@Public()` das **rohe Mongo-Dokument** mit allen `extendedOptions` + `accessGroups` zurueck. 2.1-Allowlist portiert (9 Keys) statt der 2-Eintraege-Denylist; ein Test prueft mit erfundenem Zukunfts-Key, dass Unbekanntes per Default ausgeschlossen bleibt. **Ehrliche Einordnung:** Umfang kleiner als zuerst berichtet — greift nur bei Apps mit `EMBEDDED_PAGE_IS_PUBLIC`, und die beiden Secrets des Forks haengen an Filesharing; die *Form* war trotzdem falsch.
  - `feat(auth)` (folgt) — **Drosselung auf `POST /auth` + `GET /auth/totp/:username`** (10 Versuche / 5 min, 2.1-Werte). **Fallstrick:** der Fork-`ThrottleGuard` laesst anonyme Requests ohne `byIp:true` ungedrosselt durch (`return true`) — ohne dieses Flag waere die Drosselung wirkungslos gewesen; Regression prueft genau das.

  **KORREKTUREN an der Agenten-Analyse (selbst nachgeprueft, nicht uebernommen):**
  - Behauptung „2.1.0 loescht `GET /auth/totp/:username`" ist **falsch** — 2.1.0 behaelt die Route und markiert sie „legacy, disabled when experimental auth is enabled" (main.js 68148/68149).
  - Behauptung „Fork ist hier OLD verbatim" ist **falsch** — der Fork gibt bei `getTotpInfo` nur `mfaEnabled` (Boolean) zurueck, **identisch mit 2.1.0** und **besser als 2.0.200**, das noch `{secret, createdAt}` auslieferte. Ein TOTP-Secret-Leak existiert im Fork **nicht**.
  - `public-share:`-Praefix: im Fork **gegenstandslos**, es gibt keinen public-share-SSE-Kanal.

  **NOCH OFFEN (nach Ausnutzbarkeit):**
  - **S3 OnlyOffice — Stufe 1 [x] ERLEDIGT (1b301c7dc), Stufe 2 offen.** Das Signier-Orakel ist zu: `assertDocumentUrlMatchesFile` (document.url == angefragte Datei, callbackUrl gleicher Origin + exakter Callback-Pfad) + `sanitizeOnlyOfficeConfig` (Server baut die Config neu auf; permissions/mode/user serverseitig) sind portiert, der Endpunkt liefert `{config, token}`, das FE rendert die **sanitisierte** Config. Wire-Form auf die Referenz gezogen (`@Body` + `filePath`/`fileName` als Query), `DOWNLOADS_PATH_SEGMENT` + `ONLY_OFFICE_CALLBACK_PATH` als geteilte Konstanten über alle Consumer. 14 Tests, beide Invarianten **mutationsgeprüft** (signiert der Code `clientConfig` statt der sanitisierten Config, oder steht `permissions` vor dem Spread → Test rot). Remote: eslint 3/3, api 91 Suites/732 Tests, vitest 40/188, build api, FE-tsc clean.
    **Korrektur an der ursprünglichen Formulierung:** „`canWrite` serverseitig" war irreführend — die authentifizierte Route übergibt `canWrite: true`, **exakt wie 2.0.200 (main.js:37372) und 2.1.0 (main.js:51308)**; die Bitmask-Ableitung existiert nur im public-share-Token-Endpunkt, den der Fork gar nicht hat. Ein `mode:'edit'` auf einem read-only-Share ergibt also weiterhin `permissions.edit=true`; die echte Schreibschranke ist der Callback-Upload über die WebDAV-Credentials des Users.
    **Bewusste Abweichungen von der Referenz** (damit beim 2.1.0-Port niemand darüber stolpert): `fileType: base.document?.fileType ?? ''` im Sanitizer (Diff zu OLD:41678 — nötig, damit der Rückgabetyp `IConfig` ohne Cast deklariert werden kann) · `IConfig` bleibt der geteilte Config-Typ in `libs`/`apps/api` statt eines eigenen `OnlyOfficeClientConfig` (nur `import type`, wird wegkompiliert; die Alternative erzwingt am FE-Render-Boundary wieder einen Cast) · der document-url-Origin wird gegen die callbackUrl verglichen, **nicht** gegen die eigene Server-URL — soll-treu zu OLD und NEW, als eigene Fork-Verbesserung vorgemerkt.
    **Nachtrag — ein DRITTER 2.0.200-Schutz fehlte, gefunden erst bei der Ledger-Reparatur:** `OnlyofficeService.handleCallback` prüfte **gar nichts** und gab `req.body.url` direkt an `FilesystemService.retrieveAndSaveFile` → `axios.get(url)`. Der Aufrufer bestimmte damit, welche URL der API-Server abruft; der Inhalt landete anschließend über `uploadFile` im WebDAV des Nutzers und war dort wieder herunterladbar — **SSRF mit Exfiltration**, erreichbar für jeden authentifizierten Nutzer. Portiert aus OLD:40243–40300: Token-Verifikation gegen das Document-Server-Secret, Echo-Abgleich signed↔body (status/url/key), Status-Gate über `ONLY_OFFICE_CALLBACK_STATUS`, `isAllowedOnlyOfficeDownloadUrl` (Origin-Vergleich, fail-closed ohne Konfiguration). `handleCallback` ist dafür von `static` auf Instanzmethode umgestellt. 21 Tests, drei Mutationen geprüft (Origin-Guard, Echo-Abgleich, Status-Echo — jede färbt einen Test rot).
    Der Review fand dabei einen nicht-offensichtlichen Angriff: **dasselbe Secret signiert Editor-Config und Callback-Token**, ein Config-Token wäre also als Callback-Token replaybar — blockiert allein dadurch, dass `sanitizeOnlyOfficeConfig` ein Objektliteral mit festem Top-Level-Keyset baut (kein `url`/`key`/`status`). Diese implizite Kopplung ist jetzt durch einen eigenen Test festgenagelt; wer `sanitizeOnlyOfficeConfig` je auf `...base` am Top-Level umstellt, macht die SSRF wieder auf.

    **Bewusst NICHT still mitgemacht — drei offene Punkte aus dem Review:**
    - **Status 4 (`CLOSED_WITHOUT_CHANGES`) endet im 403.** Der Document-Server schickt dabei kein `url` → `new URL('')` wirft → Origin-Guard weist ab. Das ist **kein Regress** (vorher lief es über `retrieveAndSaveFile`s `if (!url) throw` in einen 500), sondern ein Bug **des 2.0.200-Solls**, den erst 2.1.0 behebt (NEW:54551–54562: früher 200-Return **vor** der URL-Prüfung). Ist-Zustand ist mit einem Test festgehalten, damit die Stufe-2-Änderung bewusst passiert. **Vorziehbar ohne `sessionKeyService`** — eigener Task.
    - **`retrieveAndSaveFile` folgt Redirects** (`filesystem.service.ts:161`, `axios.get` ohne `maxRedirects`; axios folgt per Default bis zu 21). Ein Open Redirect auf dem Document-Server-Origin führte damit wieder auf interne Adressen. `maxRedirects: 0` wäre ein Einzeiler bei genau einem Aufrufer — **aber** ein Document-Server hinter einem umleitenden Proxy bräche damit, und das UI soll auf fremden Servern laufen. Braucht eine bewusste Entscheidung, kein Drive-by. 2.0.200 hat dasselbe Loch.
    - **`webdavShare.pathname` ohne `?.`** (`filesharing.service.ts:285` und 13 weitere Derefs in derselben Datei). `getWebdavShareFromCache` kann `undefined` liefern, `share` kommt aus `@Query`. 2.1.0 nutzt dort `webdavShare?.pathname` (NEW:52832), 2.0.200 nicht — also kein Fork-Drift, sondern ein mit 2.1.0 mitgefixter Soll-Bug. Eigener Task, gehört zum 2.1.0-Drift-Paket.

    **Stufe 2 (2.1) bleibt offen:** roher `eduApiToken` als `?token=` in der Callback-URL (landet in den Logs/im Cache des Document-Servers, und `extractToken` akzeptiert Query-Token) → ersetzt durch `eduCallbackToken` + etag-basierten Session-Key. **Achtung: dabei wird `POST /filesharing/callback` `@Public()`; niemals die `@Public()`-Hälfte ohne den Token portieren.**
  - **`byIp`-Throttle ist durch Header-Rotation umgehbar (S, kein Regress).** `main.ts` setzt `app.set('trust proxy', true)`, also stammt `request.ip` aus dem client-kontrollierten `X-Forwarded-For`. Betrifft `POST /auth` (bestand schon) und das neue `POST /auth/logout` gleichermassen — der Anti-DoS-Nutzen ist kleiner als beim Bauen angenommen. Sauber wäre `trust proxy` auf die bekannte Zahl von Proxy-Hops zu setzen statt auf `true`; das ist eine Deployment-Entscheidung (Traefik davor), deshalb hier nur notiert.
  - **Keycloak-`AxiosError` niemals stringifyen.** `AxiosError.toJSON()` trägt `config.data`, und das ist bei `revokeSession` der Form-Body **mit `client_secret`**. Heute geloggt wird nur `error.message` (secret-frei, geprüft), und der `LoggingInterceptor` schliesst `/auth/logout` von seinem `JSON.stringify`-Zweig aus. Wer daran etwas ändert, leakt das Client-Secret ins Log.
  - **Sieben Controller mit type-only `@Body()`-DTO (S, heute folgenlos).** `parent-child-pairing`, `license`, `docker`, `global-settings`, `appconfig`, `veyon`, `lmnApi` importieren ihr Body-DTO als `import type`. TypeScript löscht den Import, `design:paramtypes` steht dann auf `Object`, und eine `ValidationPipe` **überspringt** die Prüfung stillschweigend. Heute harmlos, weil an keiner dieser Routen eine Pipe hängt — wer eine ergänzt, härtet nichts und merkt es nicht. Zweimal in dieser Session passiert (T14, T27). Gegenmittel ist der Test aus `auth.controller.spec.ts`, der den Metatype **aus der Reflection** liest und die echte Pipe damit fährt.
  - **S5-Rest (L):** Zwei-Stufen-Login (Passwort bei Keycloak pruefen, BEVOR `TotpMissing` verraten wird; Cost-Parity; Session-Revocation), TOTP-Replay-Schutz (`totpLastUsedCounter`), `POST /auth/logout` mit Refresh-Token-Revocation + `sid`-Denylist. Grosse Verhaltensaenderung an der Anmeldung → eigenes, sorgfaeltiges Paket, nicht anhaengen.
  - **QR-Login-Haertung (L):** serverseitige Einmal-Session statt clientseitig gewuerfelter UUID (`LoginPage.tsx:275`), httpOnly/SameSite-Cookie, konstantzeitiger Vergleich.
  - **Whiteboard-Asset-Raumbindung (M)**, **InAppPermissionGuard (M)**, **`ENABLE_EXPERIMENTAL_AUTH`-Env (S)**.

  **ENTSCHEIDUNGEN FUER KEVIN:**
  1. **Soll-Stand wechseln?** Mail Phase 3 ist noch **nicht gebaut** (nur T9/Deps) — ein Wechsel auf 2.1.0 kostet dort **keinen Rework**. Guenstigster Moment. Gilt auch fuer alle weiteren Rekonstruktionen.
