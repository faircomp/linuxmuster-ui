...
- **P2-Pilot `p2-chat` läuft (Stand 2026-07-16)** — authored+committed: T1 Contract (25 libs), T2–T5 (Konstanten+3 Schemas), **T6 GroupsService** (36d6b024a), **T8 Pipe** (a37cfcfb8), **T13 i18n** (e721b02ba), **T14 useChatStore** (68294f7e3, vitest 6/6), **T15 ChatPage-Shell** (46b285665, 9 Dateien), **T16 Nachrichten-UI** (3b0e2888c, 14 Dateien, vitest 4/4) → **11/19 fertig**. Geparkt: **T7** (Kevins notifications-WIP → kaskadiert T9/T10/T11), **T12** (2.0 seedet Chat nicht + kein Icon-Asset → Kevin-Entscheidung). **Lokal verifizierbar:** vitest (renderToStaticMarkup; **kein @testing-library/react** → kein renderHook) + voller `tsc -p tsconfig.app.json --noEmit` + check-translations; nur crabbox-Deploy/Visual-Diff box-gated. Nächste WIP-freie FE-Task: **T17 (SSE-Realtime-Abo — ergänzt useGroupChat um CHAT_NEW_MESSAGE-Listener + addMessage-Dedupe)**. Danach T18 (Read-Status [?]). Backend-Strang T9/T10/T11 (+Controller T11) wartet auf Kevins WIP. **BLOCKER für Kevin:** **T7** (`markNotificationReadBySource`) editiert `apps/api/src/notifications/notifications.service.ts` = eine von Kevins **3 uncommitteten WIP-Dateien** → git-safety verbietet Anfassen, kein Hunk-Isolieren (`git add -p` nicht verfügbar). **Kaskade: T7 blockiert T9→T10→T11** (kompletter Chat-Service+Controller). → **Kevin: bitte notifications-WIP committen/stashen**, dann läuft der BE-Strang. Parallel weiter baubar (WIP-frei): T6 (GroupsService, tiefere Rekonstruktion), T12 (appconfig-Seed), T13 (i18n), T14 (useChatStore FE).
