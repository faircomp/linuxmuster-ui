...
- **P2-Pilot `p2-chat` — WIP-freie Arbeit ERSCHÖPFT bei 12/19 (Stand 2026-07-16), BLOCKIERT auf Kevin.** Authored+committed: T1 Contract (25 libs), T2–T5 (Konstanten+3 Schemas), **T6 GroupsService** (36d6b024a), **T8 Pipe** (a37cfcfb8), **T13 i18n** (e721b02ba), **T14 useChatStore** (68294f7e3), **T15 ChatPage-Shell** (46b285665), **T16 Nachrichten-UI** (3b0e2888c), **T17 SSE-Abo** (f50a5b5b9). **Das Rekonstruktions-Rezept ist damit end-to-end validiert** (Contract→Schemas→Pipe→Service→FE-Store/Page/UI/SSE; dump 1851 → reconcile main.js → eslint/tsc/vitest fangen Drift → frischer Review fängt Treue). **Alle 7 Rest-Tasks brauchen Kevins Aktion:**
  - `[?] T7` (`markNotificationReadBySource`) editiert `notifications.service.ts` = Kevins **uncommittetes WIP** → git-safety verbietet Anfassen. **Kaskadiert: T7→T9→T10→T11** (kompletter Backend-Service + Controller). → **Kevin: notifications-WIP committen/stashen**, dann baue ich T7→T11 autonom weiter.
  - `[?] T12` (appconfig-Seed): 2.0 seedet Chat NICHT + kein Icon-Asset → **Produktentscheidung** (2.0-treu vs. Fork-Default-Seed).
  - `[?] T18` (FE Read-Status): hängt an Spec-„Offene Frage 1" + Backend-Read-Status (T10, blockiert).
  - **Loop hält 60-min-Heartbeat** und nimmt T7→T11 automatisch auf, sobald das notifications-WIP nicht mehr uncommitted ist. **Lokal verifizierbar:** vitest (renderToStaticMarkup; kein @testing-library/react) + voller FE-`tsc --noEmit` + check-translations; crabbox-Deploy/Visual/Zwei-Session box-gated. **BLOCKER für Kevin:** **T7** (`markNotificationReadBySource`) editiert `apps/api/src/notifications/notifications.service.ts` = eine von Kevins **3 uncommitteten WIP-Dateien** → git-safety verbietet Anfassen, kein Hunk-Isolieren (`git add -p` nicht verfügbar). **Kaskade: T7 blockiert T9→T10→T11** (kompletter Chat-Service+Controller). → **Kevin: bitte notifications-WIP committen/stashen**, dann läuft der BE-Strang. Parallel weiter baubar (WIP-frei): T6 (GroupsService, tiefere Rekonstruktion), T12 (appconfig-Seed), T13 (i18n), T14 (useChatStore FE).
