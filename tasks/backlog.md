...
> **Commit B OFFEN: WikiSearchService** (main.js:71024, 228 Z., 3-Dep webdavShares/wikiFileproxyClient/lmnApiService) — search (page*size≥1000→400, findAllWikiShares→resolveAccessibleShares→scope SHARE:searchInShare / ALL:searchAll-Fan-out), resolveAccessibleShares (isTemplatedShare→resolveTemplatedShareSharePath via home-dir), resolveHomeDirectory (lmnApi getLmnApiToken+getUser, 5min-Cache), searchInShare/searchAll (fileproxy-fan-out, resolveOwningShare-Filter, Sort score/mtime/path, status OK/DEGRADED/UNAVAILABLE). Braucht NOCH: **resolveTemplatedShareSharePath** (+ dessen LMN-Deps) + isTemplatedShare + Modul-Konstanten (MAX_RESULT_WINDOW 1000/PER_FILEPROXY_CAP 100/FAN_OUT 6/home-dir-Cache). OF-1 (fileproxy im Stack?) box-gated klären — Service degradiert graceful, baubar.
