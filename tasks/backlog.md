...
- **P2-Pilot `p2-chat` bei 13/19 (Stand 2026-07-16).** Authored+committed: T1 Contract, T2–T5 (Konstanten+3 Schemas), **T6 GroupsService** (36d6b024a), **T8 Pipe** (a37cfcfb8), **T13 i18n** (e721b02ba), **T14 useChatStore** (68294f7e3), **T15 ChatPage-Shell** (46b285665), **T16 Nachrichten-UI** (3b0e2888c), **T17 SSE-Abo** (f50a5b5b9), **T9 ChatService-Kern** (4e0ce058b). Rekonstruktions-Rezept end-to-end validiert.
  ...
  - **WIP-frei gebaut (diese Session):** **T9 ChatService-Kern** (4e0ce058b) + **T10-Teil** getUnreadCounts/getReadReceipts (8c059ff62) + **T11-Teil** ChatController 5/6-Routen + Module + AppModule-Wiring (51d3c97fe). **→ Chat-Backend ist jetzt erreichbar** (5 Routen live; FE aus T14–T17 kann alles außer mark-as-read). **Damit ist die WIP-freie Arbeit ERSCHÖPFT.**
  - **Brauchen jetzt Kevins WIP** (`notifications.service.ts` committen/stashen, dann autonom in einem Rutsch): `[?] T7` (`markNotificationReadBySource`) → **T10-`markChatAsRead`** (ruft es + SSE CHAT_READ_STATUS_UPDATED) → **T11 POST `.../read`-Route** (delegiert an markChatAsRead). Plus optional das weggelassene **Throttling/Cache-Control** (Härtung).
...
### T10 — ChatService: Read-Status (Unread-Counts, Read-Receipts, Mark-Read)  [~] TEILWEISE: getUnreadCounts + getReadReceipts gebaut (8c059ff62, main.js:68938/68989 1:1; +chatReadStatusModel/groupsService im Konstruktor → 7 Deps; getUnreadCounts per-User username-scoped kein IDOR, getReadReceipts hinter getVerifiedGroup-Auth; Spec leer/aggregiert/null/mapped; eslint+isolierter tsc CLEAN, jest box-gated; Review approve). **OFFEN: `markChatAsRead`** — ruft `notificationsService.markNotificationReadBySource` (=T7) + SSE CHAT_READ_STATUS_UPDATED. **Blockiert auf Kevins notifications-WIP** (wie T7): sobald committet/gestasht, ziehe ich markChatAsRead nach.
Komponente: apps/api · Dateien: `apps/api/src/chat/chat.service.ts`, `apps/api/src/chat/chat.service.spec.ts`
...
### T11 — ChatController + ChatModule + AppModule-Wiring  [~] TEILWEISE: ChatController (5/6 Routen: GET groups/unread-counts/read-status/messages + POST messages) + ChatModule (3 Schemas via forFeature; @Global-Deps nicht importiert) + AppModule-Wiring + Controller-Spec (Delegation + isRoutePublic===false je Route) + chatMessage.schema createdAt-Typisierung (51d3c97fe). main.js:68438 gewinnt; kein @Public (globaler JWT-Guard); :conversationType via T8-Pipe. eslint+isolierter tsc (mit express-Augment) CLEAN, jest/DI box-gated. Review approve. **OFFEN: POST `.../read`-Route** (ruft markChatAsRead=T10-Rest → T7 → Kevins WIP). **Weggelassen (Fork hat keine Infra, Härtungs-Follow-up):** Throttling (@Throttle/ThrottleGuard/chatThrottleConfig) + `@Header(Cache-Control:no-store)` — beim T7-Nachziehen mitbauen oder als eigenes Hardening-Ticket.
