...
### T7 — BE: Mailcow-Admin-Service + Routen (AdminGuard)  [ ] IN ARBEIT (Teil 1/2 ✓ Service — ec9aa95b3)
...
> **Teil 2/2 OFFEN: Routen + Modul + Validatoren.** 6 Controller-Routen (getMailcowDomains/getMailcowMailboxes/createMailcowMailbox/updateMailcowMailbox/deleteMailcowMailboxes/updateMailboxAcl) auf MailsController, **jede mit `@UseGuards(AdminGuard)`**, @Body-DTOs (T6); MailcowAdminService-DI in mails.module.ts; **die T6-Härtungs-Validatoren mitportieren** (password2 @Match, password-Komplexität, local_part/domain-Regex, Array-Limits — braucht mailcowValidation-Modul + @Match-Decorator); jest-Spec (gemockter axios) + supertest (403 Nicht-Admin / 200 Admin / 401 ohne Token).
