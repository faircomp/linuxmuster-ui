...
- **P2-Pilot `p2-chat` bei 13/19 (Stand 2026-07-16).** Authored+committed: T1 Contract, T2–T5 (Konstanten+3 Schemas), **T6 GroupsService** (36d6b024a), **T8 Pipe** (a37cfcfb8), **T13 i18n** (e721b02ba), **T14 useChatStore** (68294f7e3), **T15 ChatPage-Shell** (46b285665), **T16 Nachrichten-UI** (3b0e2888c), **T17 SSE-Abo** (f50a5b5b9), **T9 ChatService-Kern** (4e0ce058b). Rekonstruktions-Rezept end-to-end validiert.
  ...
  - **NUR NOCH T18 offen** (letzte Pilot-Task, baubar): FE Read-Status (Unread-Badges/Mark-Read/Receipts + SSE CHAT_READ_STATUS_UPDATED) — Spec-OF1 = ja, Backend bereit. `[~] T12` resolved (2.0-treu, nicht seeden — Spec-OF3-Default). Härtungs-Follow-up (optional, eigenes Ticket): Chat-Throttling/Cache-Control (Fork hat keine Infra; Spec-OF2 empfiehlt weglassen im Pilot — bereits so). **→ Nach T18 ist der p2-chat-Pilot komplett.**
...
### T18 — FE Read-Status (Unread-Badges + Mark-Read + Receipts)  [ ]  (OF1 = ja: Spec empfiehlt „jetzt mitbauen"; Backend T10 fertig → baubar. Nutzt GET chat/unread-counts + GET .../read-status + POST .../read + SSE CHAT_READ_STATUS_UPDATED.)
