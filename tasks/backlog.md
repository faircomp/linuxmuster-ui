...
### T8 — FE: Mailcow-Admin-Panel (Domains/Mailboxen/ACL/Delegates)  [ ] IN ARBEIT (Teil 1/2 ✓ Store — 49ec200b8)
...
> **Teil 2/2 OFFEN: UI-Panel + Mount.** `MailcowAdminPanel.tsx` unter `Settings/AppConfig/mails/` (Fork-Konvention, admin-gated via AppConfigPage): Domains-Liste, Mailbox-CRUD-Tabelle (ScrollableTable wie ParentAssignmentPage) + Create/Edit/Delete-Dialoge (T6-DTOs bauen), ACL-Editor mit `MAILCOW_ACL_OPTIONS`; Mount in AppConfigPage/mails; FE-Rollen-Gate; i18n `mailcowAdmin.*` Panel-Labels (Spalten/Dialog-Felder) DE/EN/FR; Doku. **Delegates weglassen** (Phase-3-deferred, kein BE-Route). Verify = eslint + isolierter FE-tsc + vitest (renderToStaticMarkup statisch); Interaktion + Visual box-gated. Offene Produktfrage (Default=AppConfig-Sub-Panel) siehe Teil-1-Recherche-Notiz.
