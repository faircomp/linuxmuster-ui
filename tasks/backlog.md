...
- **P2-Pilot `p2-chat` läuft (Stand 2026-07-16)** — authored+committed: T1 Contract (25 libs), T2–T5 (Konstanten+3 Schemas), **T6 GroupsService** (36d6b024a), **T8 Pipe** (a37cfcfb8), **T13 i18n DE+EN+FR** (e721b02ba), **T14 useChatStore** (68294f7e3, vitest 6/6 LOKAL grün) → **9/19 fertig**. Geparkt: **T7** (Kevins notifications-WIP → kaskadiert T9/T10/T11), **T12** (2.0 seedet Chat nicht + kein Icon-Asset → Kevin-Entscheidung). **Entdeckung: vitest läuft LOKAL** (FE-Specs real verifizierbar; nur crabbox-Deploy/Visual-Diff bleibt box-gated). Nächste WIP-freie FE-Kette: T15 (ChatPage-Shell) → T16 (Nachrichten-UI) → T17 (SSE). Alles eslint+isolierter-tsc / check-translations / vitest CLEAN; jest/build/deploy box-gated. **BLOCKER für Kevin:** **T7** (`markNotificationReadBySource`) editiert `apps/api/src/notifications/notifications.service.ts` = eine von Kevins **3 uncommitteten WIP-Dateien** → git-safety verbietet Anfassen, kein Hunk-Isolieren (`git add -p` nicht verfügbar). **Kaskade: T7 blockiert T9→T10→T11** (kompletter Chat-Service+Controller). → **Kevin: bitte notifications-WIP committen/stashen**, dann läuft der BE-Strang. Parallel weiter baubar (WIP-frei): T6 (GroupsService, tiefere Rekonstruktion), T12 (appconfig-Seed), T13 (i18n), T14 (useChatStore FE).
