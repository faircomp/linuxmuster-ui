...
- **2.1.0-DRIFT-ANALYSE + SICHERHEITS-ARBEIT (2026-07-27)** — systematisch, 7 Achsen + 3 adversariale Sicherheits-Lupen, 141 Port-Items. Referenz `.reference/2.1.0/` neben `.reference/2.0.200/` erfasst (`fetch-reference.sh` ist dafuer jetzt versionierbar; Default bleibt 2.0.200, Versions-Pin bricht sauber ab).
  ...
  **NOCH OFFEN (nach Ausnutzbarkeit):**
  - **S3 OnlyOffice (M, Regression unter 2.0.200):** roher Keycloak-Access-Token als `?token=` in der Callback-URL an den Document-Server (landet in dessen Logs/Cache, und `extractToken` akzeptiert Query-Token) **plus** Signier-Orakel — der Fork signiert, was der Client postet; 2.0.200 hat `sanitizeOnlyOfficeConfig` + `assertDocumentUrlMatchesFile`, der Fork nicht. Stufe 1 = beide Validatoren + `canWrite` serverseitig + Token aus der URL. Stufe 2 (2.1) = `eduCallbackToken` — **Achtung: dabei wird `POST /filesharing/callback` `@Public()`; niemals die `@Public()`-Haelfte ohne den Token portieren.**
