...
const useAuthErrorHandler = <TFormValues extends Record<'password', unknown>>(
  ...
  useEffect(() => {
    const resolved = resolveAuthErrorAction(authError);

    if (resolved.action === AUTH_ERROR_ACTION.NONE) return;

    if (resolved.action === AUTH_ERROR_ACTION.TOTP_REQUIRED) {
      onTotpRequired?.();
      return;
    }

    form.setError('password' as Path<TFormValues>, { message: t(resolved.messageKey) });

    if (showQrCode) {
      toast.error(t(resolved.messageKey));
    }
  }, [authError, form]);
